...
class Pan_Tompkins_Plus_Plus():
    ...
    def rpeak_detection(self, ecg, sampling_rate):
        ...
        if sampling_rate == 200:
			...
			#If sampling_rate=200 keep frwquency 5-12 Hz otherwise 5-18 Hz
            ecg = ecg - np.mean(ecg)  
            ...
            Wn = [f1*2/sampling_rate, f2*2/sampling_rate]                         # cutoff based on sampling_rate
            N = 5                                           
        ...
        if sampling_rate != 200:
            ...
            b = interp1d(range(1, 6), [i*sampling_rate/8 for i in vector])(np.arange(1, 5.1, int_c))  
        ...
        THR_SIG = np.max(ecg_m[:2*sampling_rate+1])*1/3                 # Threshold-1 (paper) #0.33 of the max amplitude 
        THR_NOISE = np.mean(ecg_m[:2*sampling_rate+1])*1/2              #Threshold-2 (paper) # 0.5 of the mean signal is considered to be noise
        ...
        THR_SIG1 = np.max(ecg_h[:2*sampling_rate+1])*1/3               #Threshold-1
        THR_NOISE1 = np.mean(ecg_h[:2*sampling_rate+1])*1/2            #Threshold-2
        ...
        qrs_i_raw = qrs_i_raw.astype(int)
        
        return {"rpeaks": qrs_i_raw, "qrs_amp_raw": qrs_amp_raw, "qrs_c": qrs_c, "qrs_i": qrs_i}
